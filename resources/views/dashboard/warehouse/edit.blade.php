...
@section('content')
    <div class="col-md-6">
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ route('dashboard.warehouse.update', ['warehouse' => $warehouse->id], false) }}" novalidate>
                    @csrf
                    @method('PUT')

                    <div class="mb-3">
                        <div class="form-check">
                            <input type="checkbox" id="active" class="form-check-input @error('active') is-invalid @enderror" name="active" {{ old('active', $warehouse->active) ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $warehouse->name ?? '') }}" required autocomplete="name" autofocus>
                    ...
                    <div class="mb-3">
                        ...
                        <input id="code" type="text" class="form-control @error('code') is-invalid @enderror" name="code" value="{{ old('code', $warehouse->code ?? '') }}" required autocomplete="code">
                    ...
                    <div class="mb-3">
                        ...
                        <select id="currency_id" class="form-control @error('currency_id') is-invalid @enderror" name="currency_id">
                            @foreach ($currencies as $item)
                                <option value="{{ $item->id }}" {{ $item->id == $warehouse->currency_id ? 'selected' : '' }}>
                                    {{ $item->name }} ({{ $item->code }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="address" class="form-control @error('address') is-invalid @enderror" name="address" required>{{ old('address', $warehouse->address ?? '') }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="phone-inputs-container">
                            ...
                            @elseif (isset($warehouse))
                                @foreach ($warehouse->phones as $index => $phone)
                                    <div class="input-group phone-field mt-2">
                                        <input type="text" class="form-control @error('phones.*') is-invalid @enderror" name="phones[]" value="{{ $phone }}" required>
                                        @if ($index > 0)
                                            <button class="btn btn-outline-danger remove-phone-button">Remove</button>
                                        @endif
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let phoneIndex = {{ count($warehouse->phones) + 1 }};
